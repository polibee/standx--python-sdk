...
Last updated on January 15, 2026StandX Perps Authentication
...
Attach signature to request headers
{
    ...
    "x-impersonate": "<vault_id>",
    ...
}
Note: x-impersonate is optional. It is only needed when you want to act on behalf of a Vault. See Vault Impersonation below.
...
Vault Impersonation
Impersonation allows a Vault Owner to send API requests on behalf of a Vault. When impersonating, the request is executed in the Vault’s context (e.g. orders, positions, balances all belong to the Vault).

How to use
Add the x-impersonate header to your existing GET and POST requests. All other headers (authorization, body signature headers, etc.) stay the same.
...
Using curl
# GET request
curl 'https://api.standx.com/v1/<endpoint>' \
  -H 'Authorization: Bearer <token>' \
  -H 'x-impersonate: cv_xxxxxxxx'
...
Rules
Rule	Description
Permission	Only the Owner of a Vault can impersonate that Vault
Vault ID	The vault id must start with cv_ (e.g. cv_xxxxxxxx), otherwise the request is rejected
Scope	The header works on both GET and POST requests
Security Note: Requests carrying x-impersonate are authenticated with your own token and verified against Vault ownership. If the authenticated account is not the Owner of the specified Vault, the request will be rejected.

Last updated on August 11, 2026StandX Perps Authentication - EVM Example
...
Key Points
Wallet Setup: Uses ethers.js to create a wallet from a private key
Message Signing: EVM wallets sign the message directly using wallet.signMessage()
Signature Format: The signature is returned as-is from the wallet (hex string)
...
Last updated on January 13, 2026StandX Perps Authentication - Solana (SVM) Example
...
main().catch(console.error);
Key Points
Wallet Setup: Uses @solana/web3.js Keypair with a base58-encoded private key

Message Signing: Uses @noble/curves/ed25519 for Ed25519 signing with walletKeypair.secretKey.slice(0, 32) (first 32 bytes are the private key)
...
Last updated on January 13, 2026StandX Perps HTTP API List
⚠️ Please note that the API responses below are for illustrative purposes only. Actual values are subject to change based on market conditions.
...
reduce_only	boolean	Only reduce position if true. Max allowed qty = position qty - cumulative qty of other pending reduce_only orders
About reduce_only: The maximum quantity for a reduce_only order is calculated as: position qty - cumulative qty of all pending reduce_only orders. This includes TP/SL orders, which are also reduce_only orders. When placing new reduce_only orders, ensure the total quantity of all your reduce_only orders (including TP/SL) does not exceed your position size.
...
tp_price	decimal	Take-profit trigger price. Creates a TP order after this order getting filled
sl_price	decimal	Stop-loss trigger price. Creates a SL order after this order getting filled
Request Example:
...
This endpoint is asynchronous. A successful response indicates the cancellation request has been accepted, not that the order has been cancelled. To confirm cancellation, subscribe to the Order Response Stream via WebSocket.

To receive order updates, add the x-session-id header to your request. This session_id must be consistent with the session_id used in your ws-client.
...
This endpoint is asynchronous, same as Cancel Order. A successful response indicates the cancellation request has been accepted, not that the orders have been cancelled. To confirm each cancellation, subscribe to the Order Response Stream via WebSocket.
...
Wallet Operations
Cash Wallet / Perps Wallet transfers and withdraw operations are documented in Wallet Operation.
...
POST /api/withdraw
The EVM Highway transaction required after creating a withdraw request
Query Order
GET /api/query_order
...
Returns the user’s funding payment/receipt history.
...
last_id	number	Last funding transaction ID for pagination
limit	number	Results limit
Response Fields

Field	Type	Description
id	int	Funding transaction ID
user	string	User identifier
asset	string	Settlement asset (e.g., DUSD)
symbol	string	Trading pair
qty	decimal	Funding amount
txn_type	string	Transaction type
transact_time	string	Funding settlement time (ISO 8601)
created_at	string	Record creation time (ISO 8601)
updated_at	string	Record update time (ISO 8601)
Response Example:
...
Returns a summary of all supported markets, including 24h volume, open interest, funding rates, and latest prices.
...

{
  ...
  "symbols": [
    ...
    {
      "base": "ETH",
      "funding_rate": "-0.00000157",
      "last_price": "2238.30",
      "mark_price": "2238.7",
      "open_interest": "16586.2040",
      "open_interest_notional": "37131534.89480",
      "price_change_pct": 1.33991218143135,
      "quote": "DUSD",
      "symbol": "ETH-USD",
      "time": "2026-04-10T16:29:17.400234Z",
      "volume_24h": "39606.743999999984225723892442",
      "volume_quote_24h": "87325756.45479999482631683348"
    }
  ]
}
Response Fields
...
Field	Type	Description
open_interest_notional	decimal	Total open interest notional across markets
symbol_count	int	Number of supported trading pairs
volume_quote_24h	decimal	Total 24h trading volume in quote asset
symbols[]

Field	Type	Description
base	string	Base asset (e.g., BTC, ETH)
quote	string	Quote asset (e.g., DUSD)
...
open_interest	decimal	Open interest in base asset
open_interest_notional	decimal	Open interest in quote asset
price_change_pct	float	24h price change percentage
volume_24h	decimal	24h volume in base asset
volume_quote_24h	decimal	24h volume in quote asset
...
Last updated on May 19, 2026Wallet Operation
...
Bridge / Cash Wallet
The Bridge / Cash Wallet section covers on-chain deposit, Cash Wallet funding, and withdrawal through the StandX Bridge API.

Base URL: https://bridge-info.standx.com
...
POST https://bridge-info.standx.comapi/withdraw
...
Create the withdraw request with POST https://bridge-info.standx.comapi/withdraw. This reserves the off-chain Cash Wallet amount and returns:
...
Perps Wallet
The Perps Wallet section covers operations that move funds out of the Perps Wallet.

Base URL: https://perps.standx.com

Transfer Perps Wallet to Cash Wallet
POST https://perps.standx.com/api/transfer_to_spot
...
Last updated on May 18, 2026StandX Perps WebSocket API List
...
The request passed the gateway check and has been accepted for processing.

{
  "code": 0,
  "status": "accepted",
  "request_id": "bccc2b23-03dc-4c2b-4315ebbbb7e0-4315ebbbb7e0"
}
Success Response:
...
Last updated on August 11, 2026Rate Limits
...
Implement backoff logic to handle 429 responses gracefully — avoid retrying immediately.
Monitor usage and distribute requests evenly across the budget window to prevent bursts.
Note
...
Last updated on March 1, 2026StandX Perps API Reference
Enums
Symbol
