...
`MarketsApi.overview()`、`symbol_market()`、`symbol_price()`和`depth_book()`分别对应 StandX 的市场概览、标的行情、标的价格和深度接口；深度 asks/bids 保持服务端顺序，不在 SDK 内隐式排序。`MarketsApi.kline_history()`、`server_time()`和`health()`分别对应文档定义的 K 线历史、K 线服务时间和健康检查接口。K 线只接受文档列出的 resolution，响应的 `t/c/o/h/l/v` 并行数组必须等长，并映射为 `KlineHistory`/`KlineBar`；任何缺失数组、长度不一致、非有限数值或非 `ok` 状态统一返回 `PROTOCOL_ERROR`。`AccountApi.trade_snapshots()`和`funding_history()`分别映射用户成交与资金历史，手续费、成交价值、PnL 和资金费用均按文档使用 `Decimal`。
