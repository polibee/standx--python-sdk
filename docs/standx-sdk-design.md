...
`MarketsApi.overview()`、`symbol_market()`、`symbol_price()`和`depth_book()`分别对应 StandX 的市场概览、标的行情、标的价格和深度接口；深度 asks/bids 保持服务端顺序，不在 SDK 内隐式排序。市场元数据的必填字符串、整数、有限 Decimal、有限 `price_change_pct` 以及可选字符串/布尔字段按 JSON 类型严格校验，错误类型统一返回 `PROTOCOL_ERROR`，不会把错误字段静默转换或丢弃。`MarketsApi.kline_history()`、`server_time()`和`health()`分别对应文档定义的 K 线历史、K 线服务时间和健康检查接口。K 线只接受文档列出的 resolution，响应的 `t/c/o/h/l/v` 并行数组必须等长，并映射为 `KlineHistory`/`KlineBar`；任何缺失数组、长度不一致、非有限数值或非 `ok` 状态统一返回 `PROTOCOL_ERROR`。`AccountApi.trade_snapshots()`和`funding_history()`分别映射用户成交与资金历史，手续费、成交价值、PnL 和资金费用均按文档使用 `Decimal`。
...
- `MarketsApi.recent_trades()`保留文档定义的列表结果；`recent_trade_snapshots()`映射为 `RecentTrade`，将`price`、`qty`和`quote_qty`转换为 `Decimal`，并严格校验 JSON 布尔类型的 `is_buyer_taker`；缺少列表、元素不是对象或 limit/symbol 参数非法时返回明确的验证/协议错误；
...
- 未知 channel 转换为 `ValueError`；非对象 envelope、缺少 `channel`/`data`、缺失 DTO 必填字段、非法 Decimal、非布尔 `reduce_only`/`is_enabled` 或损坏层级结构统一转换为 `StandXError(code=PROTOCOL_ERROR)`；数字字符串仍按文档兼容解析为整数或 Decimal，但布尔值不会被转换成整数。
...
Market Stream 的用户 channel 必须先调用 `authenticate(token, impersonate=..., streams=...)`。SDK 发送文档定义的 `{ "auth": { "token": ..., "impersonate": ..., "streams": [{"channel": ...}] } }` 消息，并且只有收到 `channel=auth` 且 `data.code=200` 后才允许订阅 `order`、`position`、`balance`或`trade`；`streams`只能包含这四类用户 channel。重连时会先使用原认证参数重新认证，再按原顺序恢复用户订阅；连接失败保留认证上下文以便显式重连，认证失败则清除 token、impersonate、streams 和本地已认证状态，不会继续使用失效凭据或伪造认证成功。服务端使用 JSON 整数错误码拒绝认证时统一为 `StandXError(code=AUTH_FAILED)`；认证响应缺少 `code` 或 code 不是 JSON 整数时统一为 `PROTOCOL_ERROR`。
...
Order Response 响应缺少 `request_id`、session 不匹配、缺少 `code` 或 `code` 不是 JSON 整数、存在但不是字符串的 `status`/`message` 时，必须统一转换为 `StandXError(code=PROTOCOL_ERROR)`，并在可识别时保留 `request_id`；协议错误不能清理 pending request，也不能向公共 API 泄漏原生 JSON/类型转换异常。
