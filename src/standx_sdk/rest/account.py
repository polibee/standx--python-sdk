"""Documented StandX account and position queries."""
...
class AccountApi:
    ...
    async def balance(self) -> dict[str, Any]:
        return _decimalize(await self._transport.get("/api/query_balance"))

    async def positions(self, symbol: str | None = None) -> list[dict[str, Any]]:
        params = {"symbol": symbol} if symbol else None
        return _decimalize(await self._transport.get("/api/query_positions", params=params))
    ...
    async def change_margin_mode(self, symbol: str, margin_mode: str) -> dict[str, Any]:
        ...
        return await self._transport.post(
            "/api/change_margin_mode", json={"symbol": symbol, "margin_mode": margin_mode}, signed=True
        )

    async def trades(self, symbol: str | None = None) -> list[dict[str, Any]]:
        params = {"symbol": symbol} if symbol else None
        return _decimalize(await self._transport.get("/api/query_trades", params=params))

    async def funding_rates(self, symbol: str, start_time: int, end_time: int) -> list[dict[str, Any]]:
        return _decimalize(
